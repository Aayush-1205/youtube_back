...
export const createGroup = (name, userId) =>
  API.post("/api/groups/create", { name, userId });
export const addUserToGroup = (groupId, userId) =>
  API.post("/api/groups/add-user", { groupId, userId });
export const inviteUser = (groupId, email) =>
  API.post("/api/groups/invite", { groupId, email });
export const searchGroups = (name) => API.get(`/api/groups/search?name=${name}`);
